...
class JointOutcome(SortedFrozenSet):
    ...
    def __post_init__(self, *args, **kwargs):
        ...
        # self.names = []
        # self.outcomes = []
        self._sort_key = ''.join(io.rv.name for io in self) + ''.join(str(io.outcome) for io in self)
...
def generate_joint_outcomes(rvc):
    ...
    for outcomes in rvc.outcome_space:
        ...
        jos.append(JointOutcome(ios, sorted=True))
    jos = JointOutcomes(jos, sorted=True)
    return jos
